...
class GammaAgent(BaseAgent):
    ...
    async def audit_candidate(self, event: HiveEvent):
        ...
        await self.bus.publish(HiveEvent(
            ...
            source=self.name,
            payload={"url": payload.get('url', 'Unknown'), "arsenal": "Forensic Audit", "action": "Auditing Candidate", "payload": str(payload.get('type', 'Unknown'))[:50]}
        ))
            ...
            try:
                ...
                if not is_real and confidence > 0.7:
                    ...
                    await self.bus.publish(HiveEvent(
                        type=EventType.LIVE_ATTACK,
                        source=self.name,
                        payload={"url": payload.get('url', 'Unknown'), "arsenal": "False Positive Filter", "action": "Suppressed", "payload": reason[:50]}
                    ))
                    return
                ...
                if is_real:
                     ...
                     await self.bus.publish(HiveEvent(
                         type=EventType.LIVE_ATTACK,
                         source=self.name,
                         payload={"url": payload.get('url', 'Unknown'), "arsenal": "Vulnerability Confirmed", "action": f"Confirmed ({confidence:.0%})", "payload": str(payload.get('type', ''))[:50]}
                     ))
